<x-main-layout>
    ...
    <div class="orphans-view bg-white rounded shadow-sm p-3 mb-4">
        <div class="row">
            ...
            @endif
            @if ($orphan->profile->parents_orphan)
                <div class="flex flex-column col-3 mb-3">
            ...
            @endif
            <hr />
            @if ($orphan->family && $orphan->family->income_source)
                <div class="flex flex-column col-3 mb-3">
            ...
            @endif
            @if ($orphan->family && $orphan->family->housing_case)
                <div class="flex flex-column col-3 mb-3">
                    <p class="title">{{ __('حالة السكن') }}</p>
                    <p class="fw-semibold">{{ $orphan->family->housing_case }}
                    </p>
                </div>
            @endif
            ...
            @foreach ($orphan->phones as $phone)
                <div class="flex flex-column col-3 mb-3">
                    <p class="title">{{ __('رقم التيلفون') }}
                        {{ $loop->index + 1 }}</p>
                    <p class="fw-semibold">{{ $phone->phone_number }}
                    </p>
                </div>
            @endforeach

            <hr />
            @if ($orphan->guardian->research_date)
                <div class="flex flex-column mb-3">
                    <p class="title mb-1">
                        {{ __('البحث الميداني الداخلي') }}</p>
                    <p class="title">
                        {{ $orphan->guardian->research_date }}</p>
                    <p class="fw-semibold">
                        {{ $orphan->guardian->internal_research }}</p>
                </div>
                <hr />
            @endif
            @if ($orphan->guardian->notes)
                <div class="flex flex-column mb-3">
                    <p class="title mb-1">{{ __('ملاحظات') }}</p>
                    <p class="fw-semibold">
                        {{ $orphan->guardian->notes }}</p>
                </div>
        ...
        <div class="row">
            ...
            @if ($orphan->profile->registrar_name)
                <div class="flex flex-column col-3 mb-3">
                    ...
                    <p class="fw-semibold">
                        {{ $orphan->profile->registrar_name }}</p>
                </div>
            @endif
            <hr />
            @if ($orphan->profile->registrar_date)
                <div class="flex flex-column col-3 mb-3">
                    <p class="title">{{ __('تاريخ التسجيل') }}</p>
                    <p class="fw-semibold">
                        {{ $orphan->profile->registrar_date }}</p>
                </div>
            @endif
            <hr />
            ...
            <div class="d-flex justify-content-end">
                <a href="{{ url()->previous() }}"
                    class="add-button pt-2 pb-2 text-decoration-none">
                    {{ __('عرض أقل للتفاصيل') }}</a>
            </div>
        </div>
    </div>
