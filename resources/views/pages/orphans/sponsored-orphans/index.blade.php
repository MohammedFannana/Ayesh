...
<!-- استدعاء Font Awesome إذا مش مستدعيه قبل -->
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css"/>
    ...
    <div class="d-flex justify-content-between align-items-center">
        ...
        <!-- زر تحميل اكسل -->
        <a href="{{route('orphan.export.excel' , request()->query())}}" class="btn btn-success rounded-pill px-3 d-flex align-items-center gap-2" type="button">
                    ...
                    <form action="{{ route('orphan.sponsored.index') }}" method="GET" id="filterForm">
                        ...
                        <div class="d-flex flex-wrap gap-2 mb-3">
                            ...
                            <div class="d-flex flex-column">
                                @foreach(['جمعية دار البر', 'جمعية الشارقة', 'جمعية السيدة مريم', 'جمعية دبي الخيرية'] as $assoc)
                                    <div class="form-check">
                                        <input class="form-check-input checkbox_filter" type="checkbox"
                                               name="filter[]" value="{{ $assoc }}"
                                               id="{{ Str::slug($assoc) }}"
                                               @if(in_array($assoc, request()->get('filter', []))) checked @endif>
                                        <label class="form-check-label" for="{{ Str::slug($assoc) }}">
                                            {{ $assoc }}
                                        </label>
                                    </div>
                                @endforeach
                            ...
                            <div style="min-width:150px;">
                                <select name="governorate" id="governorate" class="select2-governorate form-control">
                                    <option value="">اختر المحافظة</option>
                                    @foreach($governorates as $gov)
                                        <option value="{{ $gov->name }}"
                                            @if(request('governorate') == $gov->name) selected @endif>
                                            {{ $gov->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            {{-- العمر من --}}
                            <div style="min-width:150px;">
                                <select name="age_from" id="age_from" class="select2-age-from form-control">
                                    <option value="">من العمر</option>
                                    @foreach(range(0,25) as $i)
                                        <option value="{{ $i }}" @if(request('age_from') == $i) selected @endif>
                                            {{ $i == 0 ? 'يوم واحد' : "$i سنة" }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            {{-- العمر إلى --}}
                            <div style="min-width:150px;">
                                <select name="age_to" id="age_to" class="select2-age-to form-control">
                                    <option value="">إلى العمر</option>
                                    @foreach(range(0,25) as $i)
                                        <option value="{{ $i }}" @if(request('age_to') == $i) selected @endif>
                                            {{ $i == 0 ? 'يوم واحد' : "$i سنة" }}
                                        </option>
                                    @endforeach
                                </select>
